...
if __name__ == "__main__":
    mode = "batch_predict"
    model_path = '../weights/GEAUNet_B_dataset_salt_lovaszloss/best_epoch_weights.pth'
    count = False
    name_classes = ["background", "1", "2", "3"]
    img_size = [224,224]
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    batch_origin_path = "./visualize/original_img"  # 设置测试集图片的文件夹路径
    batch_save_path = "./visualize/output_img"  # 设置预测结果保存的路径

    from ..nets.CA_EGEUNet_v2 import self_net
    model = self_net(n_classes=4,c_list=[32,64,128,64,128,256]).to(device)

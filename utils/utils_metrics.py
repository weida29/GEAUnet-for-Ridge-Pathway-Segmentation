...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None):
    ...
    for ind in range(len(gt_imgs)):
        ...
        if name_classes is not None and ind > 0 and ind % 10 == 0:
            print('{:d} / {:d}: mIoU-{:0.2f}%; mPA-{:0.2f}%; Accuracy-{:0.2f}%'.format(
                    ind,
                    len(gt_imgs),
                    100 * np.nanmean(per_class_iu(hist)[1:]),  # 忽略背景类0
                    100 * np.nanmean(per_class_PA_Recall(hist)[1:]),  # 忽略背景类0
                    100 * per_Accuracy(hist)
                )
            )
    ...
    if name_classes is not None:
        for ind_class in range(1, num_classes):  # 从1开始忽略背景类0
            print('===>' + name_classes[ind_class] + ':\tIou-' + str(round(IoUs[ind_class] * 100, 2)) \
                + '; Recall (equal to the PA)-' + str(round(PA_Recall[ind_class] * 100, 2))+ '; Precision-' + str(round(Precision[ind_class] * 100, 2)))

    print('===> mIoU: ' + str(round(np.nanmean(IoUs[1:]) * 100, 2)) +  # 忽略背景类0
          '; mPA: ' + str(round(np.nanmean(PA_Recall[1:]) * 100, 2)) +  # 忽略背景类0
